...
class MainActivity : AppCompatActivity() {
    ...
    override fun onRequestPermissionsResult(
        ...
        if(requestCode == ID_LOCATION_PERMISSION){
            if( grantResults[0] == PackageManager.PERMISSION_GRANTED ||
                ...
                Toast.makeText(this, "Access Success", Toast.LENGTH_SHORT).show()
            ...
            if(isLocationEnabled()){
                LocationServices.getFusedLocationProviderClient(this).lastLocation.addOnSuccessListener { location ->
                    ...
                    val distance = calculateDistance(
                        currentLat,
                        currentLong,
                        getAdresses()[0].latitude,
                        getAdresses()[0].longitude) * 1000
    ...
    private fun getAdresses() : List<Address>{
        val destionationPlace = "Dinas Komunikasi dan Informatika Kota Surabaya"
        val geocode = Geocoder(this, Locale.getDefault())
        return geocode.getFromLocationName(destionationPlace, 100)
    }

    private fun calculateDistance (lat1: Double, lon1: Double, lat2: Double, lon2: Double): Double{
        val r = 6372.8 // in kilometer
